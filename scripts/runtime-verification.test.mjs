...
backend | npm error command sh -c nest start --watch
...
backend | LOG [NestApplication] Nest application successfully started
...
test("probe-status formateres læsbart", () => {
  assert.equal(formatProbeProgress([
    { label: "Backend", error: null },
    { label: "Frontend", error: "fetch failed" },
  ]), "Backend: klar · Frontend: venter (fetch failed)");
});

test("runtime-resume indeholder begge HTTP-statusser", () => {
  assert.equal(
    buildRuntimeSummary({ backendStatus: 200, frontendStatus: 200, logIssueCount: 0 }),
    "Runtime-smoke OK: backend HTTP 200, frontend HTTP 200, 0 nye fatale logfejl.",
  );
});
