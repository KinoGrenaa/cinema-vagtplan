...
export type UserWriteDbClient = Prisma.TransactionClient;
...
export async function lockUserWrite(
  ...
  await transaction.$executeRaw`
    SELECT pg_advisory_xact_lock(
      ${USER_WRITE_LOCK_NAMESPACE}::integer,
      ${userId}::integer
    )
  `;
...
export async function withUserDirectoryWriteLock<T>(
  prisma: PrismaService,
  action: (transaction: UserWriteDbClient) => Promise<T>,
) {
  return prisma.$transaction(async (transaction) => {
    await transaction.$executeRaw`
      SELECT pg_advisory_xact_lock(
        ${USER_DIRECTORY_LOCK_NAMESPACE}::integer,
        0::integer
      )
    `;
