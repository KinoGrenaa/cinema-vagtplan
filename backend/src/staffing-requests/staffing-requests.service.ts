...
import { createAiEmergencyStaffingRequests } from './helpers/staffing-request-ai-flow';
import { createStaffingRequest } from './helpers/staffing-request-create-flow';
import {
...
export class StaffingRequestsService {
  ...
  async findAll(user: AuthUser, selectedCinemaId?: number | null) {
    return findAllStaffingRequests(this.prisma, user, selectedCinemaId);
  }

  async findMine(user: AuthUser, selectedCinemaId?: number | null) {
    return findMineStaffingRequests(this.prisma, user, selectedCinemaId);
  }
  ...
  async createAiEmergencyRequests(params: {
    ...
    return createAiEmergencyStaffingRequests({
      prisma: this.prisma,
      realtimeGateway: this.realtimeGateway,
      staffingAiService: this.staffingAiService,
      params,
    });
  }
}
