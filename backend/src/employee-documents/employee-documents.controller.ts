...
const employeeDocumentExtensionsByMime: Record<
  string,
  readonly string[]
> = {
  'application/pdf': ['.pdf'],
...
const employeeDocumentStoredExtensionByMime: Record<
  string,
  string
> = {
  'application/pdf': '.pdf',
...
type EmployeeDocumentUploadBody = {
  userId?: unknown;
  title?: unknown;
  cinemaId?: unknown;
};
...
export class EmployeeDocumentsController {
  ...
  private async hasExpectedFileSignature(
  ...
  private async removeRejectedUpload(
    file: Express.Multer.File,
  ) {
    try {
  ...
  findForUser(
    ...
    return this.employeeDocumentsService.findForUser(
      ...
      parseOptionalEmployeeDocumentCinemaId(cinemaId),
    );
  }
    ...
    FileInterceptor('file', {
      storage: diskStorage({
        ...
        filename: (_, file, callback) => {
          ...
          const extension =
            employeeDocumentStoredExtensionByMime[file.mimetype];

          if (!extension) {
            return callback(
              new BadRequestException(
                invalidEmployeeDocumentMessage,
              ),
              '',
            );
          }
  ...
  async uploadDocument(
    ...
    @Body() body: EmployeeDocumentUploadBody,
  ) {
    if (!file) {
    ...
    try {
      ...
      return await this.employeeDocumentsService.create(req.user, {
        userId: parseEmployeeDocumentUserId(body?.userId),
        title: normalizeEmployeeDocumentTitle(body?.title),
        cinemaId: parseOptionalEmployeeDocumentCinemaId(
          body?.cinemaId,
        ),
  ...
  async downloadDocument(
    ...
    const document =
      await this.employeeDocumentsService.getDownload(
        req.user,
        parseEmployeeDocumentId(id),
        parseOptionalEmployeeDocumentCinemaId(cinemaId),
      );
    ...
    return new StreamableFile(
      createReadStream(document.filePath),
    );
  }
