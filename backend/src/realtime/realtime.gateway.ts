...
function getAuthenticatedUser(client: Socket) {
  return (client.data as { user?: RealtimeSocketUser }).user ?? null;
}
  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleConnection(client: Socket) {
    ...
    if (!token || !jwtSecret) {
      console.warn(`Rejected unauthenticated realtime client: ${client.id}`);
      client.disconnect(true);
      return;
    }

    try {
      const payload = this.jwtService.verify<RealtimeJwtPayload>(token, {
        secret: jwtSecret,
      });
      const userId = parsePositiveNumber(payload.sub);

      if (!userId || !payload.role) {
        console.warn(`Rejected realtime client with invalid token: ${client.id}`);
        client.disconnect(true);
        return;
      }
      ...
      client.data.user = {
        id: userId,
        role: payload.role,
        cinemaId: parsePositiveNumber(payload.cinemaId),
  ...
  handleJoinCinema(
    ...
    const numericCinemaId = parsePositiveNumber(cinemaId);
    ...
    if (user.role !== 'MASTER' && user.cinemaId !== numericCinemaId) {
      console.warn(
  ...
  handleLeaveCinema(
    ...
    if (user.role !== 'MASTER' && user.cinemaId !== numericCinemaId) {
      console.warn(
        `Rejected realtime cinema room leave for client ${client.id}: cinema-${numericCinemaId}`,
      );
  ...
  handleJoinUser(
    ...
    if (!user || !numericUserId || user.id !== numericUserId) {
      return {
        joined: null,
      };
    }
