...
describe('buildPayrollXlsxExport', () => {
  it('skriver og genindlæser en lønfil med den hardenede ExcelJS-kæde', async () => {
    ...
    expect(sheet?.getCell('O1').value).toBe('Admin note');
    expect(sheet?.getCell('P1').value).toBe('Låst');
    expect(sheet?.getCell('Q1').value).toBe('Låst op af MASTER');
    expect(sheet?.getCell('O2').value).toBe('');
    expect(sheet?.getCell('P2').value).toBe('Ja');
    expect(sheet?.getCell('Q2').value).toBe('Nej');
  });
});
