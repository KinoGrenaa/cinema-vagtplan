...
const moduleNameByKey = new Map<CinemaModuleKey, string>(
  CINEMA_MODULE_CATALOG.map((module) => [module.key, module.name]),
);

export function getCinemaModuleDependencies(
  moduleKey: CinemaModuleKey,
): readonly CinemaModuleKey[] {
  return CINEMA_MODULE_DEPENDENCIES[moduleKey] ?? [];
}

export function getCinemaModuleName(moduleKey: CinemaModuleKey) {
  return moduleNameByKey.get(moduleKey) ?? moduleKey;
}
...
export function getCinemaModuleDependencyMessage(
  ...
  return `${getCinemaModuleName(moduleKey)} kræver, at ${getCinemaModuleName(
    dependencyKey,
  )} er aktiv.`;
}
