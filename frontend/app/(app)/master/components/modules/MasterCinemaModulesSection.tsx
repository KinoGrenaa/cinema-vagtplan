...
function getModuleNames(modules: CinemaModule[], moduleKeys: string[]) {
  const moduleByKey = new Map(
    modules.map((module) => [module.key, module.name]),
  );

  return moduleKeys.map((moduleKey) => moduleByKey.get(moduleKey) ?? moduleKey);
}

function collectDependentModuleKeys(
  modules: CinemaModule[],
  disabledModuleKey: string,
) {
  const disabledKeys = new Set([disabledModuleKey]);
  let changed = true;
  ...
  while (changed) {
    ...
    for (const module of modules) {
      if (disabledKeys.has(module.key)) {
        continue;
      }

      if ((module.requires ?? []).some((key) => disabledKeys.has(key))) {
        disabledKeys.add(module.key);
        changed = true;
      }
    }
  }
...
export default function MasterCinemaModulesSection({
  ...
  const [modules, setModules] = useState<CinemaModule[]>([]);
  const [initialModules, setInitialModules] = useState<CinemaModule[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchModules() {
      try {
        ...
        setError("");
        setMessage("");
        const response = await apiFetch(`/cinema-modules/${cinemaId}`);
        ...
        const data = (await response.json()) as CinemaModulesResponse;
        if (cancelled) {
          return;
        }

        setModules(data.modules);
        setInitialModules(data.modules);
      } catch (fetchError) {
        if (!cancelled) {
          setError(
  ...
  const groupedModules = useMemo(() => {
    const groups = new Map<string, CinemaModule[]>();
  ...
  const hasChanges = useMemo(() => {
    ...
    const initialByKey = new Map(
      initialModules.map((module) => [module.key, module.enabled]),
    );

    return modules.some(
      (module) => initialByKey.get(module.key) !== module.enabled,
    );
  ...
  }, [cinema, hasChanges, onClose, saving]);
  ...
  function toggleModule(moduleKey: string) {
    ...
    if (!selectedModule.enabled) {
      const missingRequirementKeys = (selectedModule.requires ?? []).filter(
        (requirementKey) =>
          modules.find((module) => module.key === requirementKey)?.enabled !==
          true,
      );

      if (missingRequirementKeys.length > 0) {
        const requirementNames = getModuleNames(
          modules,
          missingRequirementKeys,
        );
        setMessage(
          `${selectedModule.name} kræver, at ${requirementNames.join(
            " og ",
          )} er aktiv.`,
        );
        return;
    ...
    const disabledKeys = collectDependentModuleKeys(modules, moduleKey);
    const dependentKeys = [...disabledKeys].filter((key) => key !== moduleKey);

    setModules((current) =>
      current.map((module) =>
        disabledKeys.has(module.key)
          ? {
              ...module,
              enabled: false,
            }
    ...
    if (dependentKeys.length > 0) {
      const dependentNames = getModuleNames(modules, dependentKeys);
      setMessage(
        dependentNames.length === 1
          ? `${dependentNames[0]} blev også deaktiveret, fordi modulet kræver ${selectedModule.name}.`
          : `${dependentNames.join(
              ", ",
            )} blev også deaktiveret, fordi modulerne afhænger af ${selectedModule.name}.`,
      );
    } else {
      setMessage("");
    }
  ...
  async function saveModules() {
    ...
    try {
      ...
      setError("");
      setMessage("");
      const response = await apiFetch(`/cinema-modules/${cinema.id}`, {
        method: "PATCH",
        body: JSON.stringify({
      ...
      const data = (await response.json()) as CinemaModulesResponse;
      setModules(data.modules.map((module) => ({ ...module })));
      setInitialModules(data.modules.map((module) => ({ ...module })));
      window.dispatchEvent(
        new CustomEvent("cinemaModulesChanged", {
          detail: {
            cinemaId: cinema.id,
          },
        }),
    ...
    >
      ...
      >
        <header className="border-b border-gray-200 px-5 py-4 dark:border-gray-800 md:px-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-3xl text-sm text-gray-600 dark:text-gray-300">
                Angiv hvilke moduler biografen skal have adgang til. Der gemmes
                endnu ingen priser.
              </p>
            </div>
            <button
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-semibold">
            ...
            <span className="rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-gray-700 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-200">
              {modules.length - enabledCount} deaktiverede
            </span>
            {hasChanges && (
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 md:px-6">
          ...
          {message && (
            <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950/35 dark:text-amber-200">
              {message}
            </div>
          )}
                ...
                <div key={group}>
                  ...
                  <div className="grid gap-3 lg:grid-cols-2">
                    {groupModules.map((module) => {
                      ...
                      const hasMissingRequirements = (module.requires ?? [])
                        .some(
                          (requirementKey) =>
                            modules.find(
                      ...
                      return (
                        <button
                          ...
                          aria-disabled={
                            hasMissingRequirements && !module.enabled
                          }
                            ...
                            <div>
                              ...
                              {requirementNames.length > 0 && (
                                <p className="mt-2 text-xs font-semibold text-purple-700 dark:text-purple-300">
                                  Kræver: {requirementNames.join(" og ")}
                                </p>
                              )}
                              <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                Fremtidigt faktureringsgrundlag: pr. biograf
                              </p>
                            </div>
                            <span
        ...
        <footer className="flex flex-col gap-3 border-t border-gray-200 px-5 py-4 sm:flex-row sm:items-center dark:border-gray-800 md:px-6">
          <div aria-live="polite" className="min-h-5 flex-1 text-sm">
            {error && (
