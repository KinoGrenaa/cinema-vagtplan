...
export default function TradeCard({
  ...
  const approvedLeaveConflict = trade.approvedLeaveConflict ?? null;

  return (
    <article className="rounded-2xl border border-gray-200 bg-gray-50 p-4 shadow-sm transition-colors dark:border-gray-800 dark:bg-gray-950/70 md:p-5">
      ...
      <dl className="mt-4 grid gap-3 rounded-xl border border-gray-200 bg-white p-4 text-sm dark:border-gray-800 dark:bg-gray-900 sm:grid-cols-2">
        <div>
          <dt className="font-medium text-gray-500 dark:text-gray-400">Dato</dt>
          <dd className="mt-1 font-semibold text-gray-900 dark:text-gray-100">
            {formatShiftDate(trade.shift.startTime)}
          </dd>
        </div>
        <div>
          <dt className="font-medium text-gray-500 dark:text-gray-400">Tid</dt>
          <dd className="mt-1 font-semibold text-gray-900 dark:text-gray-100">
      ...
      {approvedLeaveConflict && (
        <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950 dark:border-amber-800 dark:bg-amber-950/35 dark:text-amber-100">
          <div className="font-bold">Godkendt fravær overlapper vagten</div>
          <p className="mt-1 leading-5">
            Du kan stadig acceptere vagten, men dit godkendte fravær ændres
            ikke automatisk.
          </p>
          <p className="mt-2 text-xs font-semibold text-amber-800 dark:text-amber-200">
            Fravær: {formatShiftDate(approvedLeaveConflict.startDate)} ·{" "}
            {formatShiftTime(
