...
export function usePayrollTypesData({
  ...
  const fetchPayrollTypes = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await readErrorMessage(response, "Kunne ikke hente eksportkoder"),
        );
      }
      ...
      showErrorRef.current(
        "Kunne ikke hente eksportkoder",
        getErrorMessage(error, "Eksportkoderne kunne ikke hentes. Prøv igen."),
      );
    } finally {
      setLoading(false);
    }
