...
type ScheduleTemplateWeekdayTabsProps = {
  template: ScheduleTemplateStaffingTemplate;
...
function getTemplateDay(template: ScheduleTemplateStaffingTemplate, weekday: number) {
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-7">
      {weekdays.map((weekday) => {
        const day = getTemplateDay(template, weekday.value);
        const active = selectedWeekday === weekday.value;
        const dayGapSummary = summarizeStaffingGaps(getDayStaffingGaps(day));

        return (
          <button
            key={weekday.value}
            type="button"
            onClick={() => onSelectWeekday(weekday.value)}
