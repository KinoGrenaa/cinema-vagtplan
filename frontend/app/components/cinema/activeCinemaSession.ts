...
export async function fetchActiveCinemaOptions() {
  const response = await apiFetch("/auth/default-cinema-options");
  if (!response.ok) {
    throw new Error(
      await readApiError(
        response,
  ...
  return (await response.json()) as ActiveCinemaOptionsResponse;
}

export async function switchActiveCinema(cinemaId: number) {
  ...
  if (!response.ok) {
    throw new Error(
      await readApiError(response, "Den aktive biograf kunne ikke skiftes."),
    );
  }
  return (await response.json()) as SwitchedCinemaSession;
}
